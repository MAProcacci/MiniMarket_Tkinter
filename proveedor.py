...
class Proveedores(tk.Frame):
    ...
    def widgets(self):
        ...
        self.labelframe = tk.LabelFrame(self, text="Proveedores", font=("Arial", 20, "bold"), bg=COLOR_FONDO)
        ...
        lbl_nro_id = tk.Label(self.labelframe, text="Nro. ID Corp: ", font=("Arial", 14, "bold"), bg=COLOR_FONDO)
        lbl_nro_id.place(x=10, y=100)
        self.entry_nro_id_corp = ttk.Entry(self.labelframe, font=("Arial", 14))
        self.entry_nro_id_corp.place(x=10, y=130, width=220, height=40)
        ...
        btn_modificar = tk.Button(self.labelframe, 
                                ...
                                fg="white",
                                cursor="hand2", 
                                command=self.modificar_proveedor)
        btn_modificar.place(x=10, y=470, width=220, height=40)

        # Definicion y configuracion del TreeView (Lista de Proveedores). Lado Derecho
        ...
        self.tree = ttk.Treeview(treeFrame, 
                                columns=("ID", "Nombre", "Nro. ID Corp", "Direccion", "Telefono", "Correo"),
                                show="headings",
        ...
        self.tree.heading("Nro. ID Corp", text="Nro. ID Corp")
        ...
        self.tree.column("Nro. ID Corp", width=120, anchor="center")
    ...
    def validar_campos(self):
        """Valida que todos los campos del formulario sean completados."""
        if not self.entry_nombre.get() or not self.entry_nro_id_corp.get() or not self.entry_direccion.get() or not self.entry_telefono.get() or not self.entry_correo.get():
    ...
    def agregar_proveedor(self):
        ...
        nro_id = self.entry_nro_id_corp.get()
        ...
        if not validar_correo(correo):
            messagebox.showerror("Agregar Proveedor - Error", "El correo electrónico no tiene un formato válido.")
            return

        try:
            conn = QueriesSQLite.create_connection(DB_NAME)
            query = "INSERT INTO proveedores (nombre, nro_id, direccion, telefono, correo) VALUES (?, ?, ?, ?, ?)"
            data = (nombre, nro_id, direccion, telefono, correo)
            QueriesSQLite.execute_query(conn, query, data)
            conn.close()
            messagebox.showinfo("Agregar Proveedor - Informacion", "Proveedor agregado correctamente")
            self.limpiar_treeview()
            self.limpiar_campos()
            self.cargar_registros()
        except Exception as e:
            messagebox.showerror("Agregar Proveedor - Error", f"Error al agregar el proveedor: {str(e)}")
            registrar_error(f"Agregar Proveedor - Error al agregar el proveedor: {str(e)}")
            conn.close()
    ...
    def limpiar_campos(self):
        ...
        self.entry_nro_id_corp.delete(0, tk.END)
    ...
    def modificar_proveedor(self):
        ...
        if not self.tree.selection():
            messagebox.showwarning("Modificar_Proveedor - Advertencia", "Por favor, seleccione un proveedor para modificar")
            return
        ...
        # Crear ventana emergente de modificar proveedor
        ...
        label_nro_id_nuevo = tk.Label(top_modificar, text="Nro. ID Corp: ", font=("Arial", 14, "bold"), bg=COLOR_FONDO)
        ...
        def guardar_modificaciones():
            ...
            try:
                conn = QueriesSQLite.create_connection(DB_NAME)
                query = "UPDATE proveedores SET nombre = ?, nro_id = ?, direccion = ?, telefono = ?, correo = ? WHERE id = ?"
                data = (nuevo_nombre, nuevo_nro_id, nueva_direccion, nuevo_telefono, nuevo_correo, id)
                QueriesSQLite.execute_query(conn, query, data)
                conn.close()
                messagebox.showinfo("Guardar_Modificaciones - Informacion", "Proveedor modificado correctamente")
                top_modificar.destroy()
                self.limpiar_treeview()
                self.cargar_registros()
            except Exception as e:
                messagebox.showerror("Guardar_Modificaciones - Error", f"Error al modificar el proveedor: {str(e)}")
                registrar_error(f"Guardar_Modificaciones - Error al modificar el proveedor: {str(e)}")
                conn.close()
